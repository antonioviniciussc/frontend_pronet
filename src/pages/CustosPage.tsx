...
function useTotaisAtividade(projetoId: number, atividadeId: number) {
          ...
          <thead>
            <tr className="border-b border-border">
              {[
                ...
                <th
                  key={h}
                  className="text-left px-2 py-1.5 text-text-muted font-medium"
                >
                  {h}
                </th>
              ))}
              ...
              <>
                {itens.map((it) => (
                  <tr key={it.id} className="border-b border-border/50">
                    ...
                    <td className="px-2 py-2 text-text-secondary">
                      {formatCurrency(it.custoPlanTotal)}
                    </td>
                    <td className="px-2 py-2 text-text-secondary">
                      {formatCurrency(it.custoReal)}
                    </td>
                    <td
                      className={`px-2 py-2 font-medium ${it.variacao > 0 ? "text-danger" : it.variacao < 0 ? "text-success" : "text-text-secondary"}`}
                    >
                      {it.variacao > 0 ? "+" : ""}
                      {formatCurrency(it.variacao)}
                    </td>
                ...
                <tr className="font-semibold bg-white">
                  <td
                    colSpan={3}
                    className="px-2 py-2 text-text-primary text-xs"
                  >
                    Total Materiais
                  </td>
                  <td className="px-2 py-2 text-text-secondary">
                    {formatCurrency(totalMatPlan)}
                  </td>
                  <td className="px-2 py-2 text-text-secondary">
                    {formatCurrency(totalMatReal)}
                  </td>
                  <td
                    className={`px-2 py-2 ${totalMatReal - totalMatPlan > 0 ? "text-danger" : "text-success"}`}
                  >
              ...
              <>
                {recursos.map((r) => (
                  <tr key={r.id} className="border-b border-border/50">
                    ...
                    <td className="px-2 py-2 text-text-secondary">
                      {formatCurrency(r.custoPlanTotal)}
                    </td>
                    <td className="px-2 py-2 text-text-secondary">
                      {r.hhReal}
                    </td>
                    <td className="px-2 py-2 text-text-secondary">
                      {formatCurrency(r.custoRealTotal)}
                    </td>
                    <td
                      className={`px-2 py-2 font-medium ${r.variacao > 0 ? "text-danger" : r.variacao < 0 ? "text-success" : "text-text-secondary"}`}
                    >
                      {r.variacao > 0 ? "+" : ""}
                    ...
                    <td className="px-2 py-2">
                      <button
                        onClick={() => excluirRecurso.mutate(r.id)}
                        className="text-text-muted hover:text-danger transition-colors p-0.5 rounded"
                      >
                        <Trash2 size={13} />
                      </button>
                ...
                <tr className="font-semibold bg-white">
                  <td className="px-2 py-2 text-text-primary text-xs">
                    Total HH
                  </td>
                  <td className="px-2 py-2 text-text-secondary">
                    {recursos.reduce((s, r) => s + r.hhPlanejado, 0)}
                  </td>
                  <td className="px-2 py-2 text-text-secondary">
                    {formatCurrency(totalHHPlan)}
                  </td>
                  <td className="px-2 py-2 text-text-secondary">
                    {recursos.reduce((s, r) => s + r.hhReal, 0)}
                  </td>
                  <td className="px-2 py-2 text-text-secondary">
                    {formatCurrency(totalHHReal)}
                  </td>
                  <td
                    className={`px-2 py-2 ${totalHHReal - totalHHPlan > 0 ? "text-danger" : "text-success"}`}
                  >
    ...
    <>
      ...
      >
        ...
        <td className="px-3 py-3 text-sm text-text-secondary text-right">
          {formatCurrency(totais.matPlan)}
        </td>
        <td className="px-3 py-3 text-sm text-text-secondary text-right">
          {formatCurrency(totais.hhPlan)}
        </td>
        <td className="px-3 py-3 text-sm font-medium text-text-primary text-right">
          {formatCurrency(totalPlan)}
        </td>
        <td className="px-3 py-3 text-sm text-text-secondary text-right">
          {formatCurrency(totais.matReal)}
        </td>
        <td className="px-3 py-3 text-sm text-text-secondary text-right">
          {formatCurrency(totais.hhReal)}
        </td>
        <td className="px-3 py-3 text-sm font-medium text-text-primary text-right">
...
function ProjetoAtividades({
  ...
  projetoId: number;
  projetoNome: string;
  onTotaisProjeto?: (id: number, t: TotaisProjeto) => void;
}) {
  const { data: atividades = [] } = useAtividades(projetoId);
  const totaisMap = useMemo(
    () => new Map<number, TotaisProjeto>(),
    [projetoId],
  );

  const reportar = (atividadeId: number, t: TotaisProjeto) => {
    totaisMap.set(atividadeId, t);
    const soma = Array.from(totaisMap.values()).reduce(
      (acc, v) => ({
        matPlan: acc.matPlan + v.matPlan,
        matReal: acc.matReal + v.matReal,
        hhPlan: acc.hhPlan + v.hhPlan,
        hhReal: acc.hhReal + v.hhReal,
      }),
...
export default function CustosPage() {
  ...
  const [totaisPorProjeto, setTotaisPorProjeto] = useState<
    Map<number, TotaisProjeto>
  >(new Map());
  ...
  const projetosFiltrados = useMemo(
    () =>
      projetoFiltro === "todos"
        ? projetos
        : projetos.filter((p) => String(p.id) === projetoFiltro),
  ...
  const atualizarTotaisProjeto = (id: number, t: TotaisProjeto) => {
  ...
  const totais = useMemo(() => {
    const idsFiltrados = new Set(projetosFiltrados.map((p) => Number(p.id)));
    let matPlan = 0,
      matReal = 0,
      hhPlan = 0,
      hhReal = 0;
    ...
    const variacaoPerc = totalPlan > 0 ? (variacao / totalPlan) * 100 : 0;
    const percRealizado =
      totalPlan > 0 ? Math.min((totalReal / totalPlan) * 100, 100) : 0;
    return {
      matPlan,
      matReal,
      hhPlan,
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-text-secondary text-sm mt-0.5">
            Planejado, Real e Comparativo por atividade
          </p>
        </div>
        <select
          value={projetoFiltro}
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="card p-4">
          ...
          <p className="text-xl font-bold text-primary mb-1">
            {formatCurrency(totais.totalPlan)}
          </p>
        ...
        <div className="card p-4">
          ...
          <p className="text-xl font-bold text-success mb-1">
            {formatCurrency(totais.totalReal)}
          </p>
          <div className="flex gap-3 text-xs text-text-muted">
            <span>
              Materiais:{" "}
        ...
        <div className="card p-4">
          ...
          <div className="flex items-center gap-1.5 mb-1">
            <p
              className={`text-xl font-bold ${totais.variacao > 0 ? "text-danger" : totais.variacao < 0 ? "text-success" : "text-text-primary"}`}
            >
              {totais.variacao > 0 ? "+" : ""}
              {formatCurrency(totais.variacao)}
            </p>
            {totais.variacao !== 0 && (
              <TrendingDown
                size={16}
                className={totais.variacao > 0 ? "text-danger" : "text-success"}
              />
            )}
          </div>
          <p
            className={`text-xs font-medium ${totais.variacao > 0 ? "text-danger" : totais.variacao < 0 ? "text-success" : "text-text-muted"}`}
          >
            {totais.variacaoPerc > 0 ? "+" : ""}
            {totais.variacaoPerc.toFixed(2)}%
          </p>
        </div>
          ...
          <div className="relative w-16 h-16">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-sm font-bold text-text-primary">
                {totais.percRealizado.toFixed(0)}%
              </span>
            </div>
          </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border bg-gray-50/50">
                    <th className="text-left px-4 py-2.5 table-header">
                      Atividade
                    </th>
                    <th
                      colSpan={3}
                      className="text-center px-3 py-2.5 table-header border-l border-border/50"
                    >
                      Planejado (R$)
                    </th>
                    <th
                  ...
                  <tr className="border-b border-border bg-gray-50/30 text-xs">
                    ...
                    <th className="px-3 py-1.5 text-right text-text-muted font-medium border-l border-border/50">
                      Materiais
                    </th>
                    <th className="px-3 py-1.5 text-right text-text-muted font-medium">
                      HH
                    </th>
                    <th className="px-3 py-1.5 text-right text-text-muted font-medium">
                      Total
                ...
                <tfoot>
                  <tr className="border-t-2 border-border bg-gray-50 font-semibold">
                    ...
                    <td className="px-3 py-3 text-sm text-right text-primary">
                      {formatCurrency(totais.matPlan)}
                    </td>
                    <td className="px-3 py-3 text-sm text-right text-primary">
                      {formatCurrency(totais.hhPlan)}
                    ...
                    <td className="px-3 py-3 text-sm text-right text-success">
                      {formatCurrency(totais.matReal)}
                    </td>
                    <td className="px-3 py-3 text-sm text-right text-success">
                      {formatCurrency(totais.hhReal)}
                    ...
                    <td
                      className={`px-3 py-3 text-sm text-right ${totais.variacao > 0 ? "text-danger" : "text-success"}`}
                    >
                      {totais.variacao > 0 ? "+" : ""}
                      {formatCurrency(totais.variacao)}
                    </td>
                    <td
                      className={`px-3 py-3 text-sm text-right ${totais.variacaoPerc > 0 ? "text-danger" : "text-success"}`}
                    >
                      {totais.variacaoPerc > 0 ? "+" : ""}
                      {totais.variacaoPerc.toFixed(2)}%
                    </td>
                ...
                <div>
                  ...
                  <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(totais.matReal / totais.totalReal) * 100}%`,
                      }}
                    />
                  </div>
                </div>
                <div>
                  ...
                  <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(totais.hhReal / totais.totalReal) * 100}%`,
                      }}
                    />
                  </div>
                </div>
                ...
                >
                  <div
                    ...
                    style={{
                      height:
                        totais.totalPlan > 0
                          ? `${Math.max(4, (totais.totalPlan / Math.max(totais.totalPlan, totais.totalReal)) * 72)}px`
                          : "4px",
                    }}
                  />
                </div>
                ...
                >
                  <div
                    ...
                    style={{
                      height:
                        totais.totalReal > 0
                          ? `${Math.max(4, (totais.totalReal / Math.max(totais.totalPlan, totais.totalReal)) * 72)}px`
                          : "4px",
                    }}
                  />
                </div>
            ...
            <div className="flex items-center gap-1.5">
              ...
              {totais.variacao !== 0 && (
                <TrendingDown
                  size={16}
                  className={
                    totais.variacao > 0 ? "text-danger" : "text-success"
                  }
                />
              )}
            </div>
            <p
              className={`text-xs mt-1 font-medium ${totais.variacaoPerc > 0 ? "text-danger" : totais.variacaoPerc < 0 ? "text-success" : "text-text-muted"}`}
            >
              {totais.variacaoPerc > 0 ? "+" : ""}
              {totais.variacaoPerc.toFixed(2)}%
            </p>
