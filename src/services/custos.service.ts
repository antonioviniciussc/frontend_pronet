...
export const custosService = {
  ...
  listarMateriaisPlanejado: async (idAtividade: number | string): Promise<ItemMaterialPlanejado[]> => {
  ...
  listarPerfisPlanejado: async (idAtividade: number | string): Promise<RecursoHHPlanejado[]> => {
    const { data } = await api.get<ApiListResponse<any[]>>(
